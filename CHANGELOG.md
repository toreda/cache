...
* Added the frequency admission policy (`admission.policy: 'frequency'`) backed by a deterministic Count-Min Sketch (`CountMinSketch`): at capacity, a newcomer is admitted over the main victim only when its estimated frequency wins, and a losing newcomer is rejected with `onAddRejected('admission')` (W-TinyLFU).
...
* Added `init.rng`, `init.victimSelector`, and `init.events` hooks. Exported `CacheRemoveReason`, `CacheRejectReason`, `CacheEvents`, `CacheInit`, `CacheItemInit`, `CfgPartial`, and every cfg group interface.
...
[0.1.0]: https://github.com/toreda/cache/compare/v0.0.0...v0.1.0
